...
```{r}
Data <- Data %>%
Data.sel <- select(pidp, h_eumem, h_sex_dv, h_age_dv, h_memorig)
...
Data <- Data %>%
        mutate(Data.sel,
          EU = case_when(
                 Data$h_umem == 1 ~ "1",
                 Data$h_eumem == 2 ~ "0",
                 Data$h_eumem != 1 | eumem != 2 ~ "NA"
        ))

## Recoding to create new character vector sex variable

mutate(Data.sel,
               sex = ifelse(h_sex_dv == 1, "Male",ifelse(h_sex_dv == 2, "Female", NA)))
...
group_by(dpData, EU,sex) %>%
  summarise(n = n())

## For age

group_by(dpData, EU,agegr) %>%
  summarise(n = n())
